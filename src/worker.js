...
async function handleAPI(request, url, env) {
  ...
  if (url.pathname === '/api/transcribe' && request.method === 'POST') {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'Transcription proxy failed: ' + err.message }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
...
function resolveTranscriptionEndpoint(baseUrlStr) {
...
async function transcribeExternal(request, env, corsHeaders, useAppKey) {
  let apiKey, baseUrlStr, model;
...
async function transcribeWorkersAI(request, env, corsHeaders) {
  if (!env.AI) {
...
function whisperResultToSRT(result) {
...
function vttToSRT(vtt) {
  ...
  const cues = [];
  let cur = null;
  const norm = (hms, ms) => (hms.length === 5 ? '00:' + hms : hms) + ',' + ms;
